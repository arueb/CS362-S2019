...
int cardEffect_adventurer(int drawntreasure, struct gameState *state, int currentPlayer, int cardDrawn, int z)
{
...
int cardEffect_smithy(int currentPlayer, struct gameState *state, int handPos)
{
	int i;
	//+3 Cards
...
int cardEffect_great_hall(int currentPlayer, struct gameState *state, int handPos)
{
	//+1 Card
	drawCard(currentPlayer, state);
...
int cardEffect_steward(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2, int choice3)
{
	if (choice1 == 1)
	{
		//start of bug
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case adventurer:
		cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer] - 1];//top card of hand is most recently drawn card
		returnNum = cardEffect_adventurer(drawntreasure, state, currentPlayer, cardDrawn, z);
		return returnNum;
    ...
    case smithy:
		//+3 Cards
		returnNum = cardEffect_smithy(currentPlayer, state, handPos);
		return returnNum;

    case village:
		returnNum = cardEffect_village(currentPlayer, state, handPos);
		return returnNum;
    ...
    case great_hall:
		returnNum = cardEffect_great_hall(currentPlayer, state, handPos);
		return returnNum;
    ...
    case steward:
		returnNum = cardEffect_steward(currentPlayer, state, handPos, choice1, choice2, choice3);
		return returnNum;
